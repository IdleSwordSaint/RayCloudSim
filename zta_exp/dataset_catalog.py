...
def _load_full_topology(
    config_path: str,
    role_overrides: Dict[str, str] | None = None,
    trust_overrides: Dict[str, float] | None = None,
) -> Tuple[List[Dict], List[Tuple[str, str, Dict[str, float]]]]:
    """
    Load the complete directed topology from the official configuration file.
    Uses NodeType mapping for roles unless overridden in role_overrides.
    """
    ...
    type_to_role = {
        ...
        "Edge": "edge",
        "Cloud": "cloud",
        # Fallbacks mapped conservatively
        "Fog": "relay",
        "Gateway": "relay",
    }
    ...
    for node in config["Nodes"]:
        ...
        node_type = node.get("NodeType", "Edge")
        role = role_overrides.get(name, type_to_role.get(node_type, "edge"))
        raw_cpu = float(node.get("MaxCpuFreq", 1500))
        raw_buffer = float(node.get("MaxBufferSize", 256))
        cpu = max(100.0, raw_cpu)
        mem = max(1.0, raw_buffer / 8.0)
        initial_trust = float(trust_overrides.get(name, 0.6))
        nodes.append(
            {
                "name": name,
                "role": role,
    ...
    links: List[Tuple[str, str, Dict[str, float]]] = []
    for edge in config.get("Edges", []):
        src_name = id_to_name.get(edge.get("SrcNodeID"))
        dst_name = id_to_name.get(edge.get("DstNodeID"))
        if not src_name or not dst_name:
            continue
        raw_bandwidth = float(edge.get("Bandwidth", 100.0))
        bandwidth = max(1.0, raw_bandwidth) * 1_000_000.0
        raw_latency = edge.get("Latency") if edge.get("Latency") is not None else edge.get("Delay")
        raw_latency = float(raw_latency) if raw_latency is not None else 10.0
        latency = max(0.5, raw_latency)
        links.append((src_name, dst_name, {"bandwidth": bandwidth, "latency": latency}))
...
MILAN_NODES, MILAN_LINKS = _load_full_topology(
    "eval/benchmarks/Topo4MEC/data/MilanCityCenter/config.json",
    role_overrides=_MILAN_ROLES,
...
TOPO25_NODES, TOPO25_LINKS = _load_full_topology(
    "eval/benchmarks/Topo4MEC/data/25N50E/config.json",
    role_overrides=_TOPO25_ROLES,
)
...
TOPO50_NODES, TOPO50_LINKS = _load_full_topology(
    "eval/benchmarks/Topo4MEC/data/50N50E/config.json",
    role_overrides=_TOPO50_ROLES,
)
...
TOPO100_NODES, TOPO100_LINKS = _load_full_topology(
    "eval/benchmarks/Topo4MEC/data/100N150E/config.json",
    role_overrides=_TOPO100_ROLES,
)
...
PAKISTAN_NODES, PAKISTAN_LINKS = _load_full_topology(
    "eval/benchmarks/Pakistan/data/Tuple30K/config.json",
    role_overrides=_PAKISTAN_ROLES,
