...
FEATURE_KEYS = [
    ...
    "centrality",
    "clustering",
    # NOTE: Do not leak ground-truth malicious labels into scheduling.
    # "malicious" feature removed for fairness and Zero-Trust compliance.
    "quarantine",
    "success_rate",
    "neighbor_trust",
    "neighbor_anomaly",
...
class GNNRLPolicy:
    ...
    def __init__(
        ...
        # Lightweight telemetry for analysis and plotting. Note: lengths may
        # differ because actions/epsilon are logged at selection-time (and may
        # include retries), while rewards are logged post-execution.
    ...
    def select_node_and_action(
        ...
        # Quarantine state is now managed by the environment to support
        # temporary vs. permanent policies; do not mutate here.
        if action == "partial_assignment":
            vec[FEATURE_KEYS.index("load")] *= 0.8

        # Log selection telemetry for monitoring/analysis.
        self.history["epsilon"].append(self.epsilon)
        self.history["action"].append(action)
        self.history["node"].append(node.name)
